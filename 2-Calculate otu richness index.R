...
## reading data 
## otu.nor<-read.table("otu.nor.txt",sep="\t",header=T,row.names=1)   
tree1<-read.tree("otu_rep.2.tre",text=NULL,tree.names=NULL,skip=0,comment.char="#")
## env<-read.table("env.txt",sep="\t",header=T,row.names=1)

## subset data ##
## otu.nor.i<-otu.nor[,1:52]
...
NTI.nj<-read.table("NTI.list.nj.txt", header=T, row.names=1, sep="\t")  ## NTI generated by nj tree with abundance
NRI.nj<-read.table("NRI.list.nj.txt", header=T, row.names=1, sep="\t")  ## NTI generated by nj tree with abundance

newdata<-cbind(env,otu.richness,otu.shannon,pd.1,otu.alpha,otu.evenness,otu.richness.delta,otu.richness.norm)
newdata<-cbind(newdata,NTI.nj,NRI.nj,pae)
newdata1<-newdata[1:48,]

write.table(newdata,file="newdata.txt",sep="\t")

plot(newdata$PD~newdata$div)
